...
LO.Display.prototype.drawPlayerPieces = function() {
  ...
  var pI, pJ, piece, playerColor, fieldId, fieldCoor, scope = this, interactive, doubleFieldIds = [], doubleField = false;

  for (pI = 0; pI < this.game.players.length; pI++) {
    for (pJ = 0; pJ < this.game.players[pI].pieces.length; pJ++) {
      piece = this.game.players[pI].pieces[pJ];
      if (piece.fieldId >= 0) {
        if (typeof doubleFieldIds[piece.fieldId] == "undefined") {
          doubleFieldIds[piece.fieldId] = -1;
        } else {
          doubleFieldIds[piece.fieldId] = 0;
        }
      }
    }
  }
  for (pI = 0; pI < doubleFieldIds.length; pI++) {
    if (doubleFieldIds[pI] === -1) {
      delete doubleFieldIds[pI];
    }
  }
  ...
  for (pI = 0; pI < this.game.players.length; pI++) {
    for (pJ = 0; pJ < this.game.players[pI].pieces.length; pJ++) {
      ...
      doubleField = false;
      if (typeof doubleFieldIds[piece.fieldId] != "undefined") {
        if (doubleFieldIds[piece.fieldId] === 0) {
          interactive = false;
          doubleFieldIds[piece.fieldId] = 1;
        } else if (doubleFieldIds[piece.fieldId] === 1) {
          doubleField = true;
        }
      }
      ...
      if (doubleField == true) {
        textStyle = {font: 28 + "px " + "Arial", fill: "000000"};
        text = new PIXI.Text("2", textStyle);
        text.position = { x: fieldCoor[0]-text.width/2, y: fieldCoor[1]-text.height/2 };
        this.playerPiecesClickLayer.addChild(text);

      } else {
        this.playerPiecesGfx.beginFill(0x000000);
        this.playerPiecesGfx.drawCircle(fieldCoor[0], fieldCoor[1], 15);

        this.playerPiecesGfx.beginFill(this.playerColors[playerColor]);
