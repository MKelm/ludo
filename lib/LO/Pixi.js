...
LO.Pixi.prototype.constructor = LO.Pixi;

LO.Pixi.prototype.autoResizeRenderer = function() {
...
LO.Pixi.prototype.animate = function(scope) {
  ...
  if (lo.game.run == true) {
    lo.game.update(lo.game);
  }
  scope.renderer.render(scope.stage);
  TWEEN.update();

  if (lo.userConfig.fps > -1) {
    global.setTimeout(function() { requestAnimFrame(scope.animate.curry(scope)); }, 1000 / lo.userConfig.fps );
  } else {
    requestAnimFrame(scope.animate.curry(scope));
  }
}

LO.Pixi.prototype.setLoader = function(activate) {
  if (activate === true) {
...
LO.Pixi.prototype.loadAssets = function(callback) {
  this.setLoader(true);
  var assetsToLoad = [
    "data/gfx/blank.png", "data/gfx/blank_dbg.png", "data/gfx/board.png",
    "data/gfx/piece_green.png", "data/gfx/piece_red.png",
    "data/gfx/piece_blue.png", "data/gfx/piece_yellow.png",
    "data/gfx/piece_green_block.png", "data/gfx/piece_red_block.png",
    "data/gfx/piece_blue_block.png", "data/gfx/piece_yellow_block.png",
    "data/gfx/dice_blank.png", "data/gfx/dice_throw.png"
  ];
...
LO.Pixi.prototype.setFpsCounter = function(keyEvent, scope) {
  ...
  if (typeof scope.fps == "undefined") {
    ...
    if (keyEvent === true && scope.fps[0] < 0) {
      ...
      lo.intervals["fpsCounter"] = setInterval(scope.setFpsCounter.curry(false, scope), 1000);
      keyEvent = false;
    ...
    if (keyEvent === true && scope.fps[0] > -1) {
      clearInterval(lo.intervals["fpsCounter"]);
      scope.fps[1].visible = false;
      scope.fps[0] = -1;
