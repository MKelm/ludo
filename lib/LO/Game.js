...
LO.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    ...
    case "player-piece-click":
      console.log("player-piece-click");
      break;
    case "player-dice-click":
      var validDice = this.players[event.content.playerId].throwDice();
      if (validDice == false) this.players[event.content.playerId].setActive(false);
      this.display.drawPlayerDices();
      break;
  }
}