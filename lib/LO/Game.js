  ...
  if (typeof pieceId == "number") {
    ...
    if (currentFieldId < -6 && player.dice.value == 6) {
      ...
      for (var dI = 0; dI < player.dice.value; dI++) {
        // check target field
        if (dI > 0 && blocked == false && this.checkFieldBlocked(player.color, currentFieldId) == true) {
          blocked = true;
          break;
        }
  ...
  switch (event.type) {
    ...
    case "player-cpu-turn":
      ...
      global.setTimeout(function() {
        console.log("cpu turn", playerId, player.active, player.activeMove);

        if (player.active == true) {
          if (player.activeMove == false) {
            // check moveable pieces to continue move
            var moveable = false;
            for (var i = 0; i < player.pieces.length; i++) {
              console.log("check blocked id", i, scope.checkPlayerPieceMoveBlocked(playerId, i), player.pieces);
              if (scope.checkPlayerPieceMoveBlocked(playerId, i) == false) {
                moveable = true;
                break;
              }
            }
            if (moveable == 0) player.setActive(false);
