...
LO.Game.prototype.checkPlayerPieceMoveBlocked = function(playerId, pieceId) {
  var player = this.players[playerId], currentFieldId = player.pieces[pieceId].fieldId;
  ...
  if (currentFieldId < -6 && player.dice.value == 6) {
    ...
    for (var dI = 0; dI < player.dice.value; dI++) {
      ...
      if (currentFieldId == this.board.fieldCoors.regular.length - 1) {
        ...
        if (currentFieldId < -6) return true; // field out of end bound
        currentFieldId = currentFieldId - 1;
      }
      // check target field
      if (this.checkFieldBlocked(player.color, currentFieldId) == true) {
        return true;
      }
    }
  }
...
LO.Game.prototype.performPlayerCpuTurn = function() {
  var scope = this;
  for (var playerId = 0; playerId < this.players.length; playerId++) {
    var player = this.players[playerId];
    if (player.cpu == true && player.active == true) {
      ...
      if (player.active == true) {
        if (player.activeMove == false) {
          ...
          for (var i = 0; i < player.pieces.length; i++) {
            if (this.checkPlayerPieceMoveBlocked(playerId, i) == false) {
...
LO.Game.prototype.handleEvent = function(event) {
  switch (event.type) {
    ...
    case "player-piece-move-complete":
      ...
      this.display.drawPlayerDices();
      this.display.drawPlayerPieces();
      this.performPlayerCpuTurn();
      break;
    case "player-dice-click":
      var player = this.players[event.content.playerId];
      // detect moveable pieces to deactive player if no pieces available
      var playerActive = false;
      for (var pieceId = 0; pieceId < player.pieces.length; pieceId++) {
        if (this.checkPlayerPieceMoveBlocked(event.content.playerId, pieceId) == false) {
          playerActive = true;
          break;
        }
      }
