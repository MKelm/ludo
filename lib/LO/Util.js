...
LO.Util = function(loadMode) {
  this.jsons = {};
  this.loadMode = loadMode || "nodejs";
  $.ajaxSetup({ async: false });
}

LO.Util.prototype.constructor = LO.Util;

LO.Util.prototype.getEventListener = function(obj, func) {
  return function(event) { obj[func](event); };
}

LO.Util.prototype.quit = function(delay) {
...
LO.Util.prototype.loadJSON = function(json, forceLoad) {
  var result = {};
...
LO.Util.prototype.objectLength = function(object) {
  var size = 0, key;
  for (key in object) {
...
LO.Util.prototype.time = function(type, delay) {
  var div = 1;
  if (type == "unix") {
    div = 1000;
  }
...
LO.Util.prototype.isoDateTime = function(t) {
...
LO.Util.prototype.isChance = function(p, max) {
...
LO.Util.prototype.isRectangesCollision = function(x1, y1, w1, h1, x2, y2, w2, h2) {
  return (x1 <= x2 + w2 &&
          x2 <= x1 + w1 &&
          y1 <= y2 + h2 &&
          y2 <= y1 + h1);
